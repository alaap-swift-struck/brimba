...
- **The Laws of the Base** (R1–R12) — [RULES.md](RULES.md), *machine-checked*: pinned to `shared/rules/registry.ts` and enforced by tests that read the source off disk (`web/test/rules.test.ts`, the per-worker `publish-seam.test.ts` for live-sync R1, the `gating-seam` suite for R10, plus `fetch-timeout` R11 and `cron-records` R12). Break one → the build goes red. Adding a Law requires the rule, the registry entry, and a check — all three.
