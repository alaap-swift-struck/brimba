...
class WorkersAiModel implements Model {
  readonly canActWithTools = false
  constructor(
    private ai: Ai,
    readonly name: string
  ) {}

  async complete(messages: ChatMessage[], _tools: ToolSpec[]): Promise<ModelReply> {
    // Text-only: tool messages are flattened into context; tools are not invoked.
    const msgs = messages.map((m) => ({
      role: m.role === "tool" ? "user" : m.role,
      content: m.role === "tool" ? `Tool result (${m.toolName ?? "tool"}): ${m.content}` : m.content,
    }))
    const out = (await this.ai.run(this.name as keyof AiModels, { messages: msgs })) as {
      response?: string
    }
    return { text: out.response ?? "", toolCalls: [] }
  }
}

/* -------------------------------- selection -------------------------------- */

/** The agentic model: Claude when a key is set (full tool use), else Workers AI. */
export function selectModel(env: Env): Model {
  ...
  return new WorkersAiModel(env.AI, env.WORKERS_AI_MODEL || "@cf/meta/llama-3.1-8b-instruct")
}
...
export async function cheapText(env: Env, system: string, user: string): Promise<string> {
  const out = (await env.AI.run((env.WORKERS_AI_MODEL || "@cf/meta/llama-3.1-8b-instruct") as keyof AiModels, {
    messages: [
