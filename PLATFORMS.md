...
7. **Re-run the gates**: `npm run check` (the app code is unchanged, so the Laws R1–R26
   still hold), then the platform's own smoke.

**What never changes in a port:** the app logic, the 7-worker split, the permission
spine (`requireRight`), the Laws (R1–R26), the screen engine, the glossary, the agent.
