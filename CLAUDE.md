...
- **The live registry pairs up BOTH WAYS (R15)** — every published resource reaches a listener (`web/lib/live-resources.ts`, or a reasoned `DEAF_EXEMPT` entry whose named cache keys are machine-checked), AND every registered listener has a real publisher. The second half was missing until 2026-08-25, and `data_import_batches` sat registered and idle for months with a comment politely explaining that nothing published it — a live listener that never fires looks exactly like nothing having changed yet. Paged screens stay live by reading the SAME cache keys the shell patches, not by subscribing. (`live-collections`)
