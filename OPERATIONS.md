...
- **Vars (in `workers/data-ops/wrangler.jsonc`, not secrets):** `AGENT_MODEL` (the Claude model id, default `claude-sonnet-4-6`, used only when `ANTHROPIC_API_KEY` is set) + `WORKERS_AI_MODEL` (the Workers AI model, default **`@cf/meta/llama-4-scout-17b-16e-instruct`**). The Workers AI model is the DEFAULT agent brain (the owner runs on Workers AI, not Anthropic) and it **does full tool calling** — verified live on staging: it chats, answers from real team data (calls read tools), and takes actions with confirmation. Swap the brain by editing this one var or `selectModel()` — "model is a battery". Other good swaps: `@cf/openai/gpt-oss-20b` / `gpt-oss-120b` (agentic), `@cf/moonshotai/kimi-k2.6` (frontier, premium, best chat). `cheapText` (inline jobs) uses the same var. **HISTORY / GOTCHAS:** (1) the old default `@cf/meta/llama-3.1-8b-instruct` was DEPRECATED+removed 5/30/2026 — calling it threw and crashed the agent on EVERY message (even "hi"); always check a model id is still served. (2) Workers AI models need the **OpenAI-wrapped tools format** `{type:"function",function:{…}}` (a flat shape 400s); the seam handles this. Docs: developers.cloudflare.com/workers-ai/function-calling/ + /models/llama-4-scout-17b-16e-instruct/.
