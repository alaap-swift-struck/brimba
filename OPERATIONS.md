...
- **Vars (in `workers/data-ops/wrangler.jsonc`, not secrets):** `AGENT_MODEL` (the Claude model id, default `claude-sonnet-4-6`, used only when `ANTHROPIC_API_KEY` is set) + `WORKERS_AI_MODEL` (the Workers AI model, default `@cf/meta/llama-3.3-70b-instruct-fp8-fast`). The Workers AI model is now the DEFAULT agent brain (the owner runs on Workers AI, not Anthropic) and it **does full tool calling** (it can act, not just answer). Swap the brain by editing this one var or `selectModel()` — "model is a battery". Fallback / cheaper option: `@cf/zai-org/glm-4.7-flash` (function calling + 131k context, ~5× cheaper). `cheapText` (inline jobs) always uses Workers AI. **History:** the old default `@cf/meta/llama-3.1-8b-instruct` was DEPRECATED+removed 5/30/2026 — calling it threw and crashed the agent on every message; that's why this id changed (docs: developers.cloudflare.com/workers-ai/function-calling/ + /models/llama-3.3-70b-instruct-fp8-fast/).
