...
| 5 | `dropdown-menu` (+ every `glass` popover) is translucent | inline `style={{ backgroundColor: "var(--popover)" }}` on `DropdownMenuContent` (app-shell switcher + profile, members-panel row menu) | `DropdownMenuContent` uses the `glass` utility (≈72%-opaque `--card` + backdrop-blur) with **no** `bg-popover`, so an open menu over page content is see-through (confirmed staging bug, 2026-06-17). A menu must be readable over anything: give the dropdown (and any popover used as a menu) an **opaque** `bg-popover` surface — keep the blur if wanted, but the fill must be solid. Same applies to `popover`/`hover-card` when used as menus. | **flag for the library** (app has an inline stopgap) |
| 6 | Selectable list-row (collection) | hand-built `<button>` rows in `web/components/roles-panel.tsx` (the role list) | The library `list` collection is **hover-only** — no selected/active state. Brimba needs a selectable list (pick a role, highlight it). Library version: a `selectedId`/`activeId` prop on `list` (or a new `selectable-list`) with an accessible `aria-current`/`data-[selected]` highlight that reads cleanly in light **and** dark mode and never swallows a row's leading icon. | **flag for the library** (hand-built here meanwhile) |
