...
| 13 | AgentChat **composer-native attach** (paperclip in the composer) | (host strip under the chat: Attach CSV button + chips + panel-wide drop — `agent-panel.tsx`) | The chat composer is library-owned, so the host can't put a paperclip INSIDE it. Want: an optional `onAttach`/`attachments` slot on `AgentChat`'s composer so the affordance sits where users expect it (Intercom-style). Host strip works today; this is polish. | **open — flag for the library (2026-07-06)** |
