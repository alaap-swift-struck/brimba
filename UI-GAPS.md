...
| 9 | AgentChat **step-row label wrap** | (host CSS override in `web/app/globals.css`, scoped `.agent-chat-host`) | `ToolRow`'s label span uses `truncate`, so on phones the useful half of "Set access rights for the sub-admin role" vanishes. Change the label span to `whitespace-normal break-words` (keep `min-w-0 flex-1`); the status chip stays `shrink-0` top-aligned. | **flagged for the library (2026-07-03)** — remove the host override when it ships |
| 10 | Collection header **compact one-row mobile layout** | (deliberate stacking today) | On `<sm` the count + search + filter wrap vertically. Owner's rule: left-to-right first. Wanted: one row on phones — the search input `flex-1` + a funnel icon button that opens the filter facets in a Sheet; count collapses into the search placeholder ("Search 3 roles…"). Desktop layout unchanged. | **flagged for the library (2026-07-03)** |
